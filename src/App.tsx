...
async function create(
  ...
  // Here, we'll be doing some extra setup to then use the `Validator` API down
  // below. To do so, we'll need to pass a `baseUrl` that allows us to connect
  // directly to the Tableland network (validator node) and look up table info.
  // The first step is to grab the `chainId` and then use the helper method to
  // connect to either a Tableland local, testnets, or mainnets node.
  const chainId = await signer.getChainId();
  ...
  // Get the created table ID from the create transaction.
  const { tableId } = create.txn!;
  // Now, create a connection to a Tableland `Validator` on the chain ID to look
  // up table information, such as the table's schema. (Pretend we don't already
  ...
  // Table's schema is an object with keys name, type, and constraints. Convert
  // this into a single string for each column.
 ...
 */
function App() {
  // Signer, its associated account address data, and the connect chain ID.
  ...
  // methods. The prefix & write data *can* be an empty string.
  ...
  // and also trigger a read query to update the `data`, rendered in the UI.
  ...
  const getOwnedTables = useCallback(async () => {
    if (db) {
      // Connect to the registry smart contract and look up owned tables by address.
  ...
  // Use the `resetTableData` callback to reset the table data displayed.
  useEffect(() => {
    resetTableData();
    getOwnedTables();
  }, [resetTableData, getOwnedTables]);

  // Upon pending write transactions (set by `handleWrite` & `write`), poll the
  // tx receipt. Once the validator confirms the receipt, that means the write
  // tx was seen by the Tableland validator. Thus, if the write tx was
  ...
  }, [pendingWriteTx, clearPendingTxAndRefresh]);
  ...
  // `database`, `validator`, `address`, and `chainId`. These are used
  // throughout various methods, particularly, the `Database`.
  async function handleConnect() {
    const { signer, db, validator } = await connect();
  ...
  // this, as opposed to the `handleWrite` below & `setPendingWriteTx`.
  ...
  // Handle click for reading from the table, which updates the `data` that is
  // then rendered in the UI's table.
